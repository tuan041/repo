...
open class XemphimProvider : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request : MainPageRequest): HomePageResponse {
        ...
        val map = mapOf(
            "Không Thể Bỏ Lỡ" to "div.list-vod.row.category-tabs-item:nth-child(1)",
            "Phim chiếu rạp" to "div.list-vod.row.category-tabs-item:nth-child(2)",
        )
        map.forEach {
            all.add(HomePageList(
                it.key,
                document.select(it.value).select("div.item.col-lg-3.col-md-4.col-sm-4.col-6").map { element ->
                    element.toSearchResult()
                }
            ))
        }

        document.select("div.col-sm-8.col-12").forEach {
            val title = it.select("h2").text().trim()
            val elements = it.select("div.item.col-lg-3.col-md-4.col-sm-4.col-6").map { element ->
                element.toSearchResult()
            }
    ...
    private fun Element.toSearchResult(): SearchResponse {
        val inner = this.selectFirst("div.item.col-lg-3.col-md-4.col-sm-4.col-6")
        val img = inner!!.select("img")
        val title = inner.attr("h3")
