...
class XemphimProvider : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val duration = if (document.select("div#myTabContent.tab-content").isNotEmpty())
            document.selectFirst("div.col-md-6.col-12:nth-child(1) > ul.more-info > li:nth-child(2)")!!.text()
            else document.selectFirst("div.col-md-6.col-12:nth-child(1) > ul.more-info > li:nth-child(1)")!!.text()
        ...
        return if (tvType == TvType.TvSeries) {
            ...
            val episodes = docEpisodes.select("ul.list-episodes.row > li").map {
                ...
                val episode = docEpisodes.select("ul.list-episodes.row > li > a").text().replace(Regex("[^0-9]"), "").trim().toIntOrNull()
